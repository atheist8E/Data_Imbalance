...
def set_args():
	...
	parser.add_argument("--model_path",    type = str,   default = "/home/sailoer123/Earth/Result/ResNet32_CIFAR_Naive/2021_01_08_15_07_16/")
	parser.add_argument("--model_file",    type = str,   default = "model.pth")
	parser.add_argument("--weight_file",   type = str,   default = "weight.pth")
	parser.add_argument("--model",         type = str,   default = "ResNet32_CIFAR_Confusion")
	parser.add_argument("--loss_function", type = str,   default = "Confusion Loss")
...
if __name__ == "__main__":
	...
	with open(os.path.join(os.path.join(os.path.join(args.target_path, args.model), start_time), "Experiment_Description.txt"), "w") as f:
		...
		train_loader = torch.utils.data.DataLoader(train_dataset, shuffle = True, batch_size = args.batch_size, pin_memory = True)
		test_loader = torch.utils.data.DataLoader(test_dataset, shuffle = False, batch_size = args.batch_size, pin_memory = True)

		################################################ Loss for Imbalance ################################################

		confusion = torch.load(os.path.join(args.model_path, args.weight_file))
		sample_counts = [5000, 2997, 1796, 645, 387, 232, 139, 83, 50]
		alpha = torch.tensor([sum(sample_counts) / sample_count for sample_count in sample_counts], dtype = torch.float32)
		beta = torch.tensor([sample_count / sum(sample_counts) for sample_count in sample_counts], dtype = torch.float32)
		criterion = Confusion_Loss(confusion, alpha, beta)

		################################################## Model Training ##################################################

		model = ResNet32_CIFAR(args, writer)
		checkpoint = torch.load(os.path.join(args.model_path, args.model_file))
		...
		model.fit(train_loader, criterion, optimizer, scheduler, args.num_epochs, val_loader = test_loader, warmup_epoch = -1)	 
