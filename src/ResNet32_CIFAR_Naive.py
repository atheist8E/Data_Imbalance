...
def set_args():
	...
	parser.add_argument("--loss_function", type = str,   default = "Softmax Cross Entropy Loss")
...
if __name__ == "__main__":
	...
	with open(os.path.join(os.path.join(os.path.join(args.target_path, args.model), start_time), "Experiment_Description.txt"), "w") as f:
		...
		if args.pretrained == "True":
			...
			model = ResNet32_CIFAR(args, writer).cuda(args.gpu)
		...
		model.fit(train_loader, criterion, optimizer, scheduler, args.num_epochs, val_loader = test_loader, warmup_epoch = 5)	 
